...
class BaseSettings(CommonBaseSettings):
    ...
    ALLAUTH_SOCIAL_PROVIDER_SECRET_PATH_PREFIX = 'auth-secret'  # nosec: only path to secret
    ...
    def _configure_auth(cls, settings: Settings) -> None:
        ...
        if settings['ACCOUNT_TEMPLATE_EXTENSION'] == 'html.j':
            ...
            settings['ACCOUNT_FORMS'] = {  # nosec: not passwords, just module paths
                'login': 'django_logikal.forms.allauth.LoginForm',
                'signup': 'django_logikal.forms.allauth.SignupForm',
                'reset_password': 'django_logikal.forms.allauth.ResetPasswordForm',
                'reset_password_from_key': 'django_logikal.forms.allauth.ResetPasswordKeyForm',
                'set_password': 'django_logikal.forms.allauth.SetPasswordForm',
                'change_password': 'django_logikal.forms.allauth.ChangePasswordForm',
            }
            settings['LOGIN_URL'] = 'account_auth'
            settings['LOGIN_REDIRECT_URL'] = 'account'
            settings['LOGOUT_REDIRECT_URL'] = 'account_auth'
        else:  # pragma: no cover, only used during module development
        ...
        # Allauth: routing
        settings['ACCOUNT_EMAIL_CONFIRMATION_AUTHENTICATED_REDIRECT_URL'] = 'account'
        ...
        with SecretManager() as secrets:
            for provider in settings['ALLAUTH_SOCIAL_PROVIDERS']:
                provider_package = f'allauth.socialaccount.providers.{provider}'
                if not installed(provider_package):
                    raise RuntimeError(f'Invalid auth provider "{provider}"')
                ...
                settings['SOCIALACCOUNT_PROVIDERS'][provider] = json.loads(
                    secrets[secret_path],
                )
