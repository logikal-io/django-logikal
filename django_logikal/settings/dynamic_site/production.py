...
class ProductionSettings(BaseSettings, CommonProductionSettings):
    ...
    .. note:: Secrets will be loaded from Google Secret Manager during import time. In particular,
        ...
        ``{project}-secret-key``, where ``{project}`` is the project name in the ``pyproject.toml``
        file), and the database configuration is loaded from the ``DATABASE_SECRETS_PATH`` setting
        key (defaults to ``{project}-database-secrets``), which must contain a JSON string with
        keys ``hostname``, ``port``, ``database``, ``username`` and ``password``.
    """
