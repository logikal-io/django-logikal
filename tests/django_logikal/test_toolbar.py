...
def test_toolbar(live_app_url_with_toolbar: LiveURL, browser: Browser) -> None:
    ...
    toolbar = browser.find_element(By.ID, 'djDebugRoot').shadow_root
    toolbar.find_element(By.ID, 'djDebugToolbarHandle').click()

    # Freeze undeterministic values
    timer_panel = toolbar.find_element(By.ID, 'djdt-TimerPanel')
    browser.replace_text(timer_panel.find_element(By.TAG_NAME, 'small'), 'CPU: 0.00ms (0.00ms)')

    template_panel = toolbar.find_element(By.ID, 'djdt-TemplateProfilerPanel')
    browser.replace_text(template_panel.find_element(By.TAG_NAME, 'small'), '1 calls in 0.00 ms')

    sql_panel = toolbar.find_element(By.ID, 'djdt-SQLPanel')
