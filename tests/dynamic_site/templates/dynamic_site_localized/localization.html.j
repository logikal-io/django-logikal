...
{% block main %}
  <article class="text">
    ...
    <p>
      {% trans %}
        This is a long localized text containing multiple sentences that spans more than one line
        in the source file and also in the message file.
      {% endtrans %}
    </p>
    {# Translators: This is a long gettext template test comment for translators that spans more
    than one line in the source file and also in the message file #}
    ...
    <h2>Components</h2>
    <p>
      {{ external_macros.test_namespaced(text=_('localized')) }}<br>
      {{ test_imported(text=_('localized')) }}<br>
      {{ aliased(text=_('localized')) }}<br>
